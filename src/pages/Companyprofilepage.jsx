...
export default function CompanyProfilePage() {
  ...
  const token = localStorage.getItem("token")
  const role = localStorage.getItem("role")
  const isOwnProfile = !id && role === "Company"
  ...
  const initials = profile.companyName?.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2) || "CO"
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 text-center">
              <div className="w-24 h-24 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg" 
                  style={{ background: "linear-gradient(135deg, #1e3a5f, #1d4ed8)" }}>
                  <span className="text-white font-bold text-3xl">{initials}</span>
              </div>
              <h1 className="text-xl font-bold text-gray-900 mb-1">{profile.companyName}</h1>
              ...
              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs text-gray-400 font-medium">On HireWave since</p>
            ...
            {profile.website && (
              ...
              >
                <div className="w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center group-hover:bg-indigo-100 transition-colors flex-shrink-0">
                  <span className="text-2xl">🌐</span>
                </div>
                <div className="min-w-0">
                  <p className="font-bold text-gray-900 text-sm">Website</p>
                  <p className="text-blue-500 text-xs truncate">{profile.website}</p>
                </div>
            ...
            {/* Browse Jobs Card */}
            <Link
              to={`/jobs?keyword=${encodeURIComponent(profile.companyName)}`}
              className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-3xl p-6 text-white text-center shadow-sm hover:shadow-lg transition-all block"
            >
              <p className="font-bold text-lg mb-1">View Open Positions</p>
              <p className="text-blue-100 text-sm">{profile.openJobCount ?? 0} jobs available</p>
            </Link>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Total Jobs", value: profile.jobCount ?? 0, icon: "📋", color: "text-blue-600" },
                { label: "Open Roles", value: profile.openJobCount ?? 0, icon: "🟢", color: "text-green-600" },
                { label: "Year Founded", value: new Date(profile.createdAt).getFullYear(), icon: "🏢", color: "text-indigo-600" },
              ].map(stat => (
            ...
            <div className="bg-gray-900 rounded-3xl p-7 text-white">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold mb-1">Interested in joining {profile.companyName}?</h3>
                  <p className="text-gray-400 text-sm">Browse their open positions and apply today</p>
                </div>
                <Link
                  to={`/jobs?keyword=${encodeURIComponent(profile.companyName)}`}
                  className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-500 transition-colors flex-shrink-0 ml-4"
                >
                  See Jobs
                </Link>
              </div>
